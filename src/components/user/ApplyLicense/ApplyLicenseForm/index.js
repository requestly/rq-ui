...
const ApplyLicenseForm = () => {
  ...
  const handleApplyLicense = () => {
    ...
    applyLicense({ license: licenseCode, company: company, email: email })
      .then((result) => {
        // console.log(result);
        setIsApplying(false);
        ...
        if (result.data.success) {
          submitEventUtil(
            ...
            TRACKING.ACTIONS.APPLIED_SUCCESSFULLY,
            `License Applied ${company} ${licenseCode}`
          );
          ...
          submitEventUtil(
            TRACKING.CATEGORIES.LICENSE,
            TRACKING.ACTIONS.APPLIED_UNSUCCESSFULLY,
            `License Failed ${company} ${licenseCode}`
          );
        }
  ...
  if (isLoading) {
    return (
      <SpinnerColumn customLoadingMessage="Fetching your license details" />
    );
  } else if (hasLicense) {
    return (
      <Row>

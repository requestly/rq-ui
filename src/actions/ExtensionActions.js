...
export function getRemoteRulesSettings() {
  return sendMessage({ action: "GET_REMOTE_RULES_SETTINGS" });
}
...
export function setRemoteRulesSettings(remoteRulesSettings) {
  return new Promise((resolve, reject) => {
    if (!isExtensionInstalled()) {
      reject("Please make sure your extension is up to date");
      return;
    }
    PSMH.sendMessage(
      {
        action: "SET_REMOTE_RULES_SETTINGS",
        remoteRulesSettings: remoteRulesSettings,
      },
      resolve
    );
  });
